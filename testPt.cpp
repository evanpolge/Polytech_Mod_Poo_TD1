...
void testPt(void) // Premiers test des points ___________________________________
{
	...
	// 1ers tests
	{
		// Construction par d�faut et affichage, surcharge ==
		pt p0;
		cout << "p0 = " << p0 << endl;
		assert(p0 == p0);
	...
	// D�placement et distance : on construit un triangle rectangle al�atoire et
	// on v�rifie le th�oreme de Pythagore
	{
		// pT1 : sommet de l'angle droit
		pt pT1(dRand(-1000, 1000), dRand(-1000, 1000));

		// pT2 : � l'horizontal du sommet de l'angle droit
		pt pT2(pT1);
		double dx12 = dRand(-1000, 1000);
		pT2.offset(dx12, 0);

		// pT3 � la verticale du sommet de l'angle droit
		pt pT3(pT1);
	...
	// Test du point milieu
	{
		...
		assert(EPSEQUAL(p3.euclide(p1), p1.euclide(p2) / 2.0)); // p3 est � �gale distance 
	...
	// Test closerTO
	{
		// On v�rifie closerTo en g�n�rant 3 points al�atoires p1, p2 et p3. 
		// On calcule pClose le point du segment [p1,p2] le plus proche de p3.
		// Puis on v�rifie qu'une s�lection d'autres points al�atoires du segment
		// [p1,p2] sont tous plus �loign�s.
		...
		for (int i = 0; i < 100; ++i)
		{
			...
			pt ptTest(p1);
			// On d�place ptTest sur le segment [p1,p2]. Ici on utilise [p1,p2] 
			// comme vectreur directeur
			ptTest.offset(d01*(p2.getX() - p1.getX()), d01*(p2.getY() - p1.getY()));
			double dTest = ptTest.euclide(p3);
			assert(dTest >= dOpt - EPSILON);                // EPSILON n�cessaire !
		}
	}
	// Test rotation
	{
		// La rotation d�un point ne change pas sa distance � l�origine
		for (int i = 0; i < 100; ++i)
		{
	...
	// Sym�trie axiale Y
	{
		...
		pt p2(p1); // clone
		p1.symY();
		// Un point et son sym�trique sont � la m�me distance de (0,0)
